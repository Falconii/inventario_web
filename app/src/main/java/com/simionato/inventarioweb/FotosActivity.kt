...
import com.simionato.inventarioweb.global.saveCompressedImageToGallery
    ...
    private fun enviarFotoFlow(foto: FotoModel,newUri : Uri): Flow<EstadoUpload> = flow {
        ...
        try {
            ...
            if (response.isSuccessful && response.body() != null) {
                    var mensagem_error = "";
                    try {
                        apagarFotoGaleria(applicationContext,newUri)
                        emit(EstadoUpload.Sucesso)
                    } catch (e: Exception) {
                        mensagem_error = "Foto Enviada Com Sucesso. Mas Continua No Galeria. Isso Não Atrapalha Operacionalmente"
                    }
                    if (mensagem_error == ""){
                       emit(EstadoUpload.Sucesso);
                    } else {
